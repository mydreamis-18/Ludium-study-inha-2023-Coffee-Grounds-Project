...
// 오너는 단 하나의 지갑일 수밖에 없고, 변경 가능하다.
// 금고의 접근 권한은 approve() 함수를 통해 여러 명일 수 있지만
// 금고를 배포자가 아닌 CA에게 넘겨주게 되면
// 컨트랙 내에 다른 컨트랙의 함수를 호출하는 게 아니면 CA는 트랜잭션을 발생시킬 수 없으므로
// 오너가 변경되는 경우가 아니면 금고의 접근 권한은 변경될 수 없다.
// 따라서 배포 당시 금고(CA)의 접근 권한을 오너에게 넘겨주었다면
// 금고(CA)에 대한 접근은 오로지 오너만 가능하며
// 오너와 금고(CA)에 대한 접근 권한은 분리될 수 없다.
...
// File: Coffeebak_Emission_v1.0.sol

pragma solidity ^0.8.19;

contract Coffeebak_Emission is Ownable {
    //
    Incheon_Coffeebak_Token public ICT_token;
    ...
    function add_cafe_emission_data(
        ...
        transfer_token(
            _cafe_wallet,
            _coffeebak_amount * coffeebak_insentive_unit, deadline, v, r, s
        );
        return true;
    }
    ...
    function transfer_token(
        ...
        //
        ICT_token.permit(address(ICT_token), address(this), _token_amount, deadline, v, r, s);
        ICT_token.transferFrom(address(ICT_token), _to, _token_amount);
    }
}
